				...
				-->
					...
					<div class="actions">
						<div class="like">
							<MkButton
								...
								@click="unlike()"
								v-if="page.isLiked"
								v-tooltip="$ts._pages.unlike"
								primary
								><i class="fas fa-heart"></i
								><span class="count" v-if="page.likedCount > 0">{{
									page.likedCount
								}}</span></MkButton
							>
							<MkButton
					...
					<div class="links">
						...
						<template v-if="$i && $i.id === page.userId">
							...
							<button v-else @click="pin(true)" class="link _textButton">
								{{ $ts.pin }}
							</button>
						</template>
					</div>
...
export default defineComponent({
	...
	methods: {
		...
		shareWithNote() {
			os.post({
				initialText: `${this.page.title || this.page.name} ${url}/@${
					this.page.user.username
				}/pages/${this.page.name}`,
			});
		},
