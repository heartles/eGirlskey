<template>
	...
	>
		...
		<div class="info" v-if="pinned">
			<i class="fas fa-thumbtack"></i> {{ $ts.pinnedNote }}
		</div>
		<div class="info" v-if="appearNote._prId_">
			<i class="fas fa-bullhorn"></i> {{ $ts.promotion
			}}<button class="_textButton hide" @click="readPromo()">
				{{ $ts.hideThisNote }} <i class="fas fa-times"></i>
			</button>
		</div>
		<div class="info" v-if="appearNote._featuredId_">
			<i class="fas fa-bolt"></i> {{ $ts.featured }}
		</div>
		<div class="renote" v-if="isRenote">
			...
			<div class="info">
				...
				<span class="localOnly" v-if="note.localOnly"
			...
			<div class="main">
				...
				<div class="body">
					...
					>
						...
						<div class="renote" v-if="appearNote.renote">
							<XNotePreview :note="appearNote.renote" />
						</div>
						<button
							v-if="collapsed"
							class="fade _button"
				...
				<footer class="footer">
					...
					<button @click="reply()" class="button _button">
						...
						<p class="count" v-if="appearNote.repliesCount > 0">
					...
					>
						<i class="fas fa-retweet"></i>
						<p class="count" v-if="appearNote.renoteCount > 0">
...
export default defineComponent({
	components: {
		...
		MkUrlPreview: defineAsyncComponent(
			() => import("@client/components/url-preview.vue")
		),
		MkInstanceTicker: defineAsyncComponent(
			() => import("@client/components/instance-ticker.vue")
		),
	},
	...
	computed: {
		...
		canRenote(): boolean {
			return (
				["public", "home"].includes(this.appearNote.visibility) || this.isMyNote
			);
		},
		...
		showTicker() {
			...
			if (
				this.$store.state.instanceTicker === "remote" &&
				this.appearNote.user.instance
			)
				return true;
			return false;
		},
	...
	async created() {
		...
		this.muted = await checkWordMute(
			this.appearNote,
			this.$i,
			this.$store.state.mutedWords
		);
	...
	methods: {
		...
		capture(withHandler = false) {
			if (this.$i) {
				...
				this.connection.send(document.body.contains(this.$el) ? "sr" : "s", {
		...
		decapture(withHandler = false) {
			if (this.$i) {
				...
				if (withHandler)
					this.connection.off("noteUpdated", this.onStreamNoteUpdated);
			}
		},
						...
						? [
								null,
								{
									...
									action: () => {
										...
										os.popup(
											import("@client/components/abuse-report-window.vue"),
											{
