<template>
	...
	>
		<div class="auth _section">
			<div
				class="avatar"
				:style="{ backgroundImage: user ? `url('${user.avatarUrl}')` : null }"
				v-show="withAvatar"
			></div>
			<div class="normal-signin" v-if="!totpLogin">
				...
				>
					...
					<template #desc
						><button class="_textButton" @click="resetPassword">
							{{ $ts.forgotPassword }}
						</button></template
					>
				</MkInput>
				<MkButton
					...
					:disabled="signing"
					style="margin: 0 auto"
					>{{ signing ? $ts.loggingIn : $ts.login }}</MkButton
				>
			</div>
			<div
				class="2fa-signin"
		...
		<div class="social _section">
			<a
				...
				><i class="fab fa-twitter" style="margin-right: 4px"></i
...
export default defineComponent({
	...
	methods: {
		...
		onSubmit() {
			...
			if (!this.totpLogin && this.user && this.user.twoFactorEnabled) {
				...
				os.api("signin", {
					...
					password: this.password,
					token:
						this.user && this.user.twoFactorEnabled ? this.token : undefined,
				})
					.then((res) => {
						this.$emit("login", res);
						this.onLogin(res);
		...
		resetPassword() {
			os.popup(
				import("@client/components/forgot-password.vue"),
				{},
				{},
				"closed"
			);
