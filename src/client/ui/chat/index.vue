	...
	<div class="mk-app" @contextmenu.self.prevent="onContextmenu">
		...
		<div class="nav">
			<header class="header">
				...
				<div class="right">
					<MkA class="item" to="/my/messaging" v-tooltip="$ts.messaging"
						><i class="fas fa-comments icon"></i
						><span v-if="$i.hasUnreadMessagingMessage" class="indicator"
							><i class="fas fa-circle"></i></span
					></MkA>
					<MkA class="item" to="/my/messages" v-tooltip="$ts.directNotes"
						><i class="fas fa-envelope icon"></i
						><span v-if="$i.hasUnreadSpecifiedNotes" class="indicator"
							><i class="fas fa-circle"></i></span
					></MkA>
					<MkA class="item" to="/my/mentions" v-tooltip="$ts.mentions"
						><i class="fas fa-at icon"></i
						><span v-if="$i.hasUnreadMentions" class="indicator"
							><i class="fas fa-circle"></i></span
					></MkA>
					<MkA class="item" to="/my/notifications" v-tooltip="$ts.notifications"
			...
			<div class="body">
				<div class="container">
					...
					<div class="body">
						<MkA
							to="/timeline/home"
							class="item"
							:class="{ active: tl === 'home' }"
							><i class="fas fa-home icon"></i>{{ $ts._timelines.home }}</MkA
						>
						<MkA
							to="/timeline/local"
							class="item"
							:class="{ active: tl === 'local' }"
							><i class="fas fa-comments icon"></i
							>{{ $ts._timelines.local }}</MkA
						>
						<MkA
							to="/timeline/social"
							class="item"
							:class="{ active: tl === 'social' }"
							><i class="fas fa-share-alt icon"></i
							>{{ $ts._timelines.social }}</MkA
						>
						<MkA
							to="/timeline/global"
							class="item"
							:class="{ active: tl === 'global' }"
							><i class="fas fa-globe icon"></i>{{ $ts._timelines.global }}</MkA
						>
					</div>
				</div>
				<div class="container" v-if="followedChannels">
					<div class="header">
						{{ $ts.channel }} ({{ $ts.following }})<button
							class="_button add"
							@click="addChannel"
						>
					...
					<div class="body">
						<MkA
							...
							class="item"
							:class="{
								active: tl === `channel:${channel.id}`,
								read: !channel.hasUnreadNote,
							}"
							><i class="fas fa-satellite-dish icon"></i>{{ channel.name }}</MkA
						>
					</div>
				</div>
				<div class="container" v-if="featuredChannels">
					...
					<div class="body">
						<MkA
							...
							:to="`/channels/${channel.id}`"
							class="item"
							:class="{ active: tl === `channel:${channel.id}` }"
							><i class="fas fa-satellite-dish icon"></i>{{ channel.name }}</MkA
						>
					</div>
				</div>
				<div class="container" v-if="lists">
					<div class="header">
						{{ $ts.lists
						}}<button class="_button add" @click="addList">
							<i class="fas fa-plus"></i>
						</button>
					</div>
					<div class="body">
						<MkA
							...
							:to="`/my/list/${list.id}`"
							class="item"
							:class="{ active: tl === `list:${list.id}` }"
							><i class="fas fa-list-ul icon"></i>{{ list.name }}</MkA
						>
					</div>
				</div>
				<div class="container" v-if="antennas">
					<div class="header">
						{{ $ts.antennas
						}}<button class="_button add" @click="addAntenna">
							<i class="fas fa-plus"></i>
						</button>
					</div>
					<div class="body">
						<MkA
							...
							:key="antenna.id"
							:to="`/my/antenna/${antenna.id}`"
							class="item"
							:class="{ active: tl === `antenna:${antenna.id}` }"
							><i class="fas fa-satellite icon"></i>{{ antenna.name }}</MkA
						>
					</div>
				</div>
				<div class="container">
					<div class="body">
						<MkA to="/my/favorites" class="item"
							><i class="fas fa-star icon"></i>{{ $ts.favorites }}</MkA
						>
					</div>
				</div>
			</div>
			<footer class="footer">
				...
				<div class="right">
					<button
						class="_button item search"
						@click="search"
						v-tooltip="$ts.search"
					>
						<i class="fas fa-search"></i>
					</button>
					<MkA class="item" to="/settings" v-tooltip="$ts.settings"
		...
		<main class="main" @contextmenu.stop="onContextmenu">
			<header class="header" ref="header" @click="onHeaderClick">
				<div class="left">
					...
					<template v-else-if="tl.startsWith('channel:')">
						...
						<div class="title" v-if="currentChannel">
							{{ currentChannel.name }}
							<div class="description">{{ currentChannel.description }}</div>
						</div>
					</template>
				</div>
				...
				<div class="right">
					...
					<button
						class="_button button timetravel"
						@click="timetravel"
						v-tooltip="$ts.jumpToSpecifiedDate"
					>
						<i class="fas fa-calendar-alt"></i>
					</button>
					<button
						class="_button button search"
						v-if="tl.startsWith('channel:') && currentChannel"
						@click="inChannelSearch"
						v-tooltip="$ts.inChannelSearch"
					>
						<i class="fas fa-search"></i>
					</button>
					<button
					...
					<button
						class="_button button follow"
						v-if="tl.startsWith('channel:') && currentChannel"
						:class="{ followed: currentChannel.isFollowing }"
						@click="toggleChannelFollow"
						v-tooltip="currentChannel.isFollowing ? $ts.unfollow : $ts.follow"
					>
						<i v-if="currentChannel.isFollowing" class="fas fa-star"></i>
						<i v-else class="far fa-star"></i>
					</button>
					<button
						class="_button button menu"
						v-if="tl.startsWith('channel:') && currentChannel"
			...
			<XTimeline
				class="body"
				ref="tl"
				v-if="tl.startsWith('channel:')"
				src="channel"
				:key="tl"
				:channel="tl.replace('channel:', '')"
			/>
		...
		<XSide
			class="side"
			ref="side"
			@open="sideViewOpening = true"
			@close="sideViewOpening = false"
		/>
...
export default defineComponent({
			...
			() => {
				if (this.tl.startsWith("channel:")) {
					os.api("channels/show", {
						channelId: this.tl.replace("channel:", ""),
					}).then((channel) => {
	...
	methods: {
		...
		async inChannelSearch() {
			...
			router.push(
				`/search?q=${encodeURIComponent(query)}&channel=${
